...
-(void)rec
{
    ...
    NSMutableArray *resizedSrcImgList = [NSMutableArray arrayWithCapacity: 1];
    
    for (int i=0; i<5; i++) {
        /* UIImagePickerなどから画像を取得 */
        UIImage *aImage = [srcImageList objectAtIndex:i];
        ...
        [resizedSrcImgList addObject:resizedImage];
    }
    
    srcImageArray = [ NSMutableArray arrayWithCapacity: 1];
    for (int j=0; j<5; j++) {
        for(int i=0; i<10; i++){
            [srcImageArray  addObject:[resizedSrcImgList objectAtIndex:j]];
        }
    }
    NSLog(@"画像配列セット完了");
...
-(void)writeImagesToMovieAtPath:(NSString *)path withSize:(CGSize)size
{
    ...
    [writerInput_ requestMediaDataWhenReadyOnQueue:dispatchQueue usingBlock:^{
        while ([writerInput_ isReadyForMoreMediaData])
        {
            ...
            CVPixelBufferRef buffer = (CVPixelBufferRef)[self pixelBufferFromCGImage:[[srcImageArray objectAtIndex:i] CGImage] andSize:size_];
            NSLog(@"bufferを書き込んだ");
            BOOL append_ok = NO;
            if (buffer){
...
//動画ファイルのパスと出力ファイルのパスを指定して動画と音楽の合成を行なう
-(void) addAudioToFileAtPath:(NSString *) videoFilePath toPath:(NSString *)outputFilePath
{
    NSError * error = nil;
